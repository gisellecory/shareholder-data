...
if os.path.isfile(local.meta_master) == False:
    metadata_master = pd.DataFrame(columns=["co_numb","category","date","doc_url"])
else:
    # Read in master metadata file
    metadata_master = pd.read_csv(local.meta_master, dtype={'co_numb': object, 'downloaded': np.int32, 'category': object, 'doc_url': object, 'date': object }, low_memory=False)
    ...
    for _filename in source_files:
        if _filename.endswith('.csv'):
            ...
            # Open
            temp_df = pd.read_csv(str(local.source_meta_dir)+"/"+str(_filename), dtype={'co_numb': object, 'category': object, 'doc_url': object, 'date': object, 'description': object, 'json': object, 'type': object, 'url': object, 'count_items': object}, low_memory=False)
