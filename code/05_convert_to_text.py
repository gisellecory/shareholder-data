...
text_output = str(fp.text_route_file) + str(today) + ".csv"
...
while instance <10000 :
    ...
    if 'doc_url' in text_df.columns:
        text_df = text_df[['doc_url']]
        text_df.to_csv(fp.index_text_output, index=False, mode="a", header=False)
        print("URLs appended to index file")

    # Delete temp files

    temp_files = os.listdir(fp.temp_dir)
